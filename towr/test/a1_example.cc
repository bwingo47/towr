...
int main()
{
    ...
    formulation.final_base_.lin.at(towr::kPos) << 0.0, 0.0, 0.309489;
    ...
    Eigen::Vector3d com_pos_init = formulation.initial_base_.lin.at(kPos);
    Eigen::Vector3d com_pos_final = formulation.final_base_.lin.at(kPos);
    Eigen::Vector3d com_pos_mid(0.0, 0.0, 0.6);

    double com_pos_init_z = com_pos_init(2);

    double launch_phase_duration = 0.15;
    double aerial_phase_duration = 0.7;
...
//    }
  ...
  //            double t_lerp = (idx*time_step_)/launch_phase_duration;
  //            x_wbd.at(i).com_pos = com_pos_init + t_lerp*(com_pos_init - com_pos_init);
  ...
  //            double t_lerp = (idx*time_step_ - aerial_phase_duration - launch_phase_duration)/landing_phase_duration;
  //            x_wbd.at(i).com_pos = com_pos_init + t_lerp*(com_pos_final - com_pos_init);
      } else {
...
//        params.ee_phase_durations_.push_back(gait_gen_->GetPhaseDurations(total_duration, ee));
        params.ee_phase_durations_.push_back({0.15, 0.7, 0.15});
...
//    for (auto c : formulation.GetNodeTrackingCosts()) // NodeTrackingCost only works with finite-diff derivative evaluation
        nlp.AddCostSet(c);
//    nlp.PrintCurrent();
